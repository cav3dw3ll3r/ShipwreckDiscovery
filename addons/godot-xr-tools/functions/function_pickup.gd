...
func _on_grab_entered(target: Node3D) -> void:
	...
	if not target.has_method('pick_up'):
		print("[PICKUP] grab_entered: ignoring target without pick_up:", target, " layers=", target.collision_layer, " mask=", target.collision_mask)
		return

	print("[PICKUP] grab_entered: target=", target, " layers=", target.collision_layer, " mask=", target.collision_mask)
...
func _update_closest_object() -> void:
	...
	if new_closest_obj != closest_object:
		print("[PICKUP] closest_object changed: old=", closest_object, " new=", new_closest_obj)
...
func drop_object() -> void:
	...
	var _throw_lin: Vector3 = _velocity_averager.linear_velocity() * impulse_factor
...
func _pick_up_object(target: Node3D) -> void:
	...
	if is_instance_valid(picked_up_object):
		if picked_up_object.has_method("request_highlight"):
