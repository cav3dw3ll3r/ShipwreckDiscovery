...
func physics_movement(delta: float, player_body: XRToolsPlayerBody, disabled: bool):
	...
	if disabled or !enabled:
		if is_active:
			print("[XRCLIMB] disabled or not enabled; turning off climb, is_active was ", is_active)
	...
	if _dominant:
		print("[XRCLIMB] active; dominant=", _dominant, " left_handle=", _left_handle, " right_handle=", _right_handle)
	...
	print("[XRCLIMB] move; offset=", offset, " distance=", distance, " new_pos=", player_body.global_position)
...
func _on_left_picked_up(what : Node3D) -> void:
	# Accept XRToolsClimbable or any object that implements the climbable interface (e.g. SolidClimbableSpotlight)
	if not what.has_method("get_grab_handle"):
		return

	# Get the handle
	_left_handle = what.get_grab_handle(_left_pickup_node)
	if not _left_handle:
		return
...
func _on_right_picked_up(what : Node3D) -> void:
	...
	# Get the handle
	_right_handle = what.get_grab_handle(_right_pickup_node)
