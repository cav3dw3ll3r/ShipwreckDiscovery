...
var _agent_debug_driver_logs: int = 0
...
func _physics_process(delta : float) -> void:
	...
	if not is_instance_valid(primary.by):
		if is_instance_valid(target) and target is XRToolsPickable:
			(target as XRToolsPickable).abort_grab_after_grabber_freed(self)
		else:
			discard()
		return

	var secondary_active := is_instance_valid(secondary) and is_instance_valid(secondary.by)
	...
	match state:
		State.LERP:
			...
			if lerp_time < lerp_duration:
				...
				if secondary_active: secondary.set_arrived()
