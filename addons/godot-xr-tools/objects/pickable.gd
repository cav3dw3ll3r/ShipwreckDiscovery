...
## Called when the grab driver must stop because the grabber node was freed
## (e.g. during scene changes) and normal [method let_go] cannot run.
func abort_grab_after_grabber_freed(driver: XRToolsGrabDriver) -> void:
	if _grab_driver != driver:
		return
	var pgrab := driver.primary
	var sgrab := driver.secondary
	_grab_driver = null
	for g in [pgrab, sgrab]:
		if g == null:
			continue
		g._clear_hand_pose()
		if is_instance_valid(g.collision_hand):
			remove_collision_exception_with(g.collision_hand)
			g.collision_hand.remove_collision_exception_with(self)
		if is_instance_valid(g.by):
			released.emit(self, g.by)
	freeze = restore_freeze
	collision_mask = original_collision_mask
	collision_layer = original_collision_layer
